...
func QueueCommand(ctx framework.Context) {
    ...
    queue := sess.Queue
    q := queue.Get()
    if len(q) == 0 && queue.CurrentSong() == nil {
        ctx.Reply("Song queue is empty! Add a song with `-add`.")
        return
    }
    buff := bytes.Buffer{}
    if queue.CurrentSong() != nil {
        buff.WriteString(fmt.Sprintf(current_format, queue.CurrentSong().Title))
    }
    queueLength := len(q)
    if len(ctx.Args) == 0 {
        var resp string
...
func display(queue []framework.Song, buff bytes.Buffer, page, start int) string {
    ...
    buff.WriteString(response_footer)
    return buff.String()
}